...
static constant DURATION = 1200		// simulation time (ms)
...
static constant FREQ_INC = 50			// MF frequency increase (Hz)
static constant NUM_FREQ = 20			// number of simulations (running # SIMULATIONS each)
// END definitions
...
Function run()
	...
	wavestokill += wavelist("gAMPA_*", ";", "")
	wavestokill += wavelist("gNMDA_*", ";", "")
...
Static Function/WAVE run_model(frequency)
	variable frequency
...
Function sort_into_folder(frequency)
	Variable frequency
...
Static Function NM_make_Conduct_Waves(sumFreq) // code for generating MF-GC conductance waves
	...
	String directList1 = WT_NMPulseExp_GC_AMPAdirect( select = 0 ) + WT_NMPulseTrainRP_GC_AMPAdirect()
	String directList2 = WT_NMPulseExp_GC_AMPAdirect( select = 1 ) + WT_NMPulseTrainRP_GC_AMPAdirect()
	String spillList1 = WT_NMPulseExp_GC_AMPAspill( select = 0 ) + WT_NMPulseTrainRP_GC_AMPAspill()
	String spillList2 = WT_NMPulseExp_GC_AMPAspill( select = 1 ) + WT_NMPulseTrainRP_GC_AMPAspill()
	String spillList3 = WT_NMPulseExp_GC_AMPAspill( select = 2 ) + WT_NMPulseTrainRP_GC_AMPAspill()
